"""Default configuration data for Mason Agent: DEFAULT_CONFIG and 
...
Pure-data leaf module — must not import from mason_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["mason-cli"],
    # journal_mode: SQLite journal mode for every Mason DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `mason -c`: each CLI session writes a breadcrumb under
        # $MASON_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `mason
        ...
        # Mason-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Mason (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env MASON_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `mason doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`mason status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # MASON_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else MASON_HOME/cache/terminal (auto-pruned
        ...
        # MASON_HOME/home; "real" = force real HOME; "profile" = force MASON_HOME/home when it
        ...
        # ["/home/user/.mason/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Mason image,
        # which drops to `mason` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `mason tools`, not a ring
        # member). Set by the `mason tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Mason-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Mason' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.mason/browser-profile/ so credentials
        ...
        # runs `mason browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `mason chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.mason/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `mason checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .mason.md,
    ...
    # Same bound for single-query mode (``mason -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Mason' summarizer cannot shrink it. native = codex decides; mason = Mason' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `mason model` → auxiliary → Curator.
    ...
    "display": {
        ...
        # Interface bare `mason`/`mason chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / MASON_TUI=1 forces the TUI.
        "interface": "cli",
        # `mason --tui` auto-resumes the most recent human-facing session (like `mason -c`).
        # MASON_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = MASON_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/mason`.
        ...
        # CLI/TUI/desktop, managed with `mason pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_mason_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: MASON_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than MASON_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env MASON_DASHBOARD_OAUTH_CLIENT_ID / MASON_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env MASON_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # MASON_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env MASON_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/mason) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.mason/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "stt": {
        ...
        "local": {
            "model": "medium",  # Custom Agent default: medium (native), was base  # tiny, base, small, medium, large-v3
    ...
    # "Hey Mason" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_mason" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.mason/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.mason/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <mason_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.mason/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # MASON_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.mason/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `mason skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${MASON_SKILL_DIR} / ${MASON_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `mason skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.mason/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.mason/.curator_backups/blobs/); powers `mason
    ...
    # daemon. `mason curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `mason curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `mason update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `mason curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.mason/skills/ to
        # ~/.mason/skills/.curator_backups/<utc-iso>/skills.tar.gz (`mason curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); mason-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "⚕ *Mason Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (MASON_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export MASON_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. MASON_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `mason plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `mason plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.mason/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # MASON_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .mason config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `mason doctor --ack <id>`; remove by editing the list. Catalog:
        # mason_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $MASON_HOME/plugins/ <name>/),
        ...
        # Env override: MASON_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: MASON_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: MASON_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: MASON_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `mason -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `mason kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with mason-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Mason tools
    # (terminal, file tools, todo, memory, browser_*, ...) are NEVER deferred.
    "tools": {
        "tool_search": {
            # Mason cold start: with no explicit `defer` list, EVERYTHING defers
            # except `clarify` (tier 1 = bridge + name:summary manifest, ~1.2k tok).
            # Tier 0 (no deferrable tools) = everything eager; tier 2 (manifest over
            # budget) = bare bridge + one-line-per-server summary. "auto"|"on" =
            # activate when at least one deferrable tool exists. "off" = pass-through.
            # Explicit `defer: [...]` keeps legacy membership semantics ([] = eager).
            # Session working set (tools/working_set.py) re-injects used tools natively.
            "enabled": "auto",
    ...
    "logging": {  # File logging to ~/.mason/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://mason-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `mason model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Mason
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "mason-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to MASON_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to MASON_STARTUP_WATCHDOG / MASON_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.mason/sessions/sessions.json mirror of the routing index
        ...
        # (MASON_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # MASON_GATEWAY_MAX_STARTS / MASON_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.mason/.env, auth.json). True:
        # files must be under the Mason cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to MASON_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Mason cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to MASON_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to MASON_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # MASON_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.mason/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `mason sessions prune`).
        ...
        # processes).
        "min_interval_hours": 24,
        # Legacy ~/.mason/sessions/session_{sid}.json snapshots rewritten every turn. state.db is
        ...
        # stay until `mason sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `mason update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.mason/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to MASON_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`mason sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `mason doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <MASON_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``mason backup`` zip in
        # <MASON_HOME>/backups/ (``mason import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `mason update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `mason update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <MASON_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `mason tools`.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.mason/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.mason/cache/,
            ...
            # Auto-download bws into ~/.mason/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Mason sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Mason also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.mason/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Mason Desktop (Electron) launch options; only affect `mason desktop`.
        ...
        # Bridged to MASON_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to MASON_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "local_runtime": {
        # Off = detection-only (Mason still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Mason releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Mason mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'mason model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Mason auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Mason' wake word engine (optional; "
        "sherpa-onnx is the free default)", "Picovoice access key",
    ...
    "MASON_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "MASON_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "MASON_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Mason, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @mason:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. MASON_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #mason)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: mason-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'mason-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Mason API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Mason API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Mason API server (proxy mode). Must "
    ...
    # MASON_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. MASON_TOOL_PROGRESS is unsupported.
    "MASON_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "MASON_EPHEMERAL_SYSTEM_PROMPT": _setting(
