"""Mason update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``mason_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``mason_cli.main`` at call time.
"""
...
from mason_cli.config import get_mason_home  # noqa: F401  (re-exported; patched via update_cmd)
from mason_cli.update_cmd_common import _best_effort
from mason_constants import get_default_mason_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from mason_cli.update_abort_recovery import (  # noqa: F401
...
from mason_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _mason_holder_subcommand, _holder_value_flags,
...
from mason_cli.update_cmd_fleet import (  # noqa: F401
...
from mason_cli.update_cmd_zip import (  # noqa: F401
...
from mason_cli.update_cmd_stash import (  # noqa: F401
...
from mason_cli.update_cmd_config import (  # noqa: F401
...
from mason_cli.update_cmd_deps import (  # noqa: F401
...
from mason_cli.update_cmd_git import (  # noqa: F401
...
from mason_cli.update_cmd_maint import (  # noqa: F401
    ...
    _print_verified_update_completion, _purge_stale_mason_modules, _read_project_version,
...
def _m():
    """Lazy ``mason_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from mason_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "mason_cli/main.py", "mason_cli/config.py", "mason_cli/__init__.py",
    "mason_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "mason_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from mason_cli.update_receipt import record_step
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="mason-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from mason_constants import get_mason_home  # noqa: F811  (deliberate: constants variant)
    home = get_mason_home()
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live Mason launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["mason.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Mason")
    print("  Desktop, a gateway, or another mason REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `mason` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_mason_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-mason.exe gate."""
    shim = scripts_dir / "mason.exe"
    lines = [
        "✗ Another mason.exe is running:",
        *(f"    PID {pid}  {name}" for pid, name in matches),
        "",
        f"  Updating now would fail to overwrite {shim} because",
        "  Windows blocks REPLACE on a running executable.",
        "",
        "  Close Mason Desktop, exit any open `mason` REPLs, and",
        "  stop the gateway (`mason gateway stop`) before retrying.",
        ""]
    if matches:
        ...
        lines += [
            ...
            ""]
    lines += [
        "  Override with `mason update --force` if you've already",
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from mason_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _cmd_update_check(branch: str = "", *, branch_explicit: bool = False):
    ...
    from mason_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from mason_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    _write_update_incomplete_marker()
    from mason_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all Mason windows/gateways and re-run: mason update")
        return False
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    from mason_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by mason.exe...")
    elif not healthy:
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running Mason gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/mason-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``mason update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in mason_cli/config.py bricked
    # every user who ran ``mason update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``mason update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway mason.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from mason_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Mason runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from mason_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another mason.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_mason_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print("  curl -fsSL https://mason-agent.nousresearch.com/install.sh | bash")
        sys.exit(1)
    ...
    if not origin_url:
        print("✗ Self-update is disabled: this Mason checkout has no 'origin' remote.")
        print('  Point it at your Mason repo (git remote add origin <url>) and re-run.')
        raise SystemExit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``mason update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``mason
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && mason update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            if _m()._is_windows():
                ...
                print(
                    ...
                    '"from mason_cli.main import main; main()" update --yes')
...
def _finalize_receipt(status: str, debug_message: str) -> None:
    ...
    with _best_effort(debug_message):
        from mason_cli.update_receipt import finalize_update_receipt
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `mason serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `mason
