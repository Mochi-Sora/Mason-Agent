...
Split out of ``mason_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
from mason_cli import _early_recovery as _early_recovery_mod

# Log-record parity with the origin module.
logger = logging.getLogger("mason_cli.main")
...
# Install-scoped breadcrumbs live next to the venv (not under $MASON_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``mason acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``mason update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from mason_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `mason update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``mason.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_mason_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from mason.exe; applying package-only first aid, "
    ...
    try:
        from mason_cli import _install_repair as _ir
        ...
        manual = (
            "  Mason is still running from the launcher that needs "
            "replacing. Close other Mason windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\mason.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``mason.exe`` never matches.
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _mason_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_mason_launcher_locked() -> bool:
    """True when a venv ``mason*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "MASON_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``mason update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\mason.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    if shim is None:
        return False
    from mason_constants import venv_python_path
    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "mason_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from mason_constants import project_venv_dir
...
def _venv_scripts_dir() -> Path | None:
    ...
    from mason_cli.main import PROJECT_ROOT
    from mason_constants import project_venv_dir, venv_bin_dir
...
def _mason_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_mason_exe(
    scripts_dir: Path, *, max_attempts: int = 4, failed_out: list[str] | None = None
) -> list[tuple[Path, Path]]:
    """Pre-empt the Windows file lock on the running ``mason.exe``.
    ...
    transient; Mason Desktop backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _mason_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Mason Desktop, exit other `mason` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `mason update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Mason versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _mason_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_mason_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``mason``
...
class ShimQuarantineError(RuntimeError):
    """A live ``mason*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``mason.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``mason.exe``
    can be neither deleted nor overwritten, so without quarantine ``mason`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_mason_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `mason` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``mason.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``mason.exe`` missing while ``mason.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``mason.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``mason.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `mason update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from mason_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``mason*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves mason.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``mason.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``mason`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``mason-agent.exe`` and ``mason-acp.exe`` present but ``mason.exe`` missing, so
    ``mason`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `mason update --force` after "
            "closing other mason processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m mason_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``mason update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `mason update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `mason update --force` after closing other mason processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `mason update --force` after closing other mason processes.",
...
def _is_termux_env(env: dict[str, str] | None = None) -> bool:
    from mason_cli.main import _is_termux_startup_environment
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from mason_constants import find_node_executable
