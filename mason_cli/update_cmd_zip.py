"""ZIP-download fallback for ``mason update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``mason_cli.update_cmd.<name>`` keeps
...
logger = logging.getLogger("mason_cli.update_cmd")

_ZIP_STAGING_ARTIFACT_SUFFIXES = ".mason-update-staging", ".mason-update-old"
...
_STASH_HINT = "  Stash or commit your changes, then rerun `mason update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``mason_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.mason-update-staging"
    backup = f"{dst}.mason-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.mason-update-staging`` siblings that git
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    See #87304.
    """
    from mason_cli.update_cmd import _m
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``mason-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"mason-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, mason_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Mason.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="mason-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"mason-agent-{branch}.zip")
        ...
        print("  Re-run `mason update` to retry; if the agent won't start, reinstall from your Mason repo (MASON_REPO_URL)")
        _m().sys.exit(1)
    finally:
...
def _reinstall_python_deps_after_zip(active_tool_dependencies) -> None:
    ...
    from mason_cli.update_cmd import (
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if not import_ok:
        ...
        print("  Re-run `mason update` to complete it.")
        _m().sys.exit(1)
    ...
    with _best_effort('Model catalog seed during zip update failed: %s'):
        from mason_cli.model_catalog import seed_cache_from_checkout
