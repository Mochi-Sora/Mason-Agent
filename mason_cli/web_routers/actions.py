"""Gateway restart/drain, Mason update and background-action status dashboard routes.

Extracted from ``mason_cli.web_server``; helpers/state that tests monkeypatch on
...
from mason_cli import __version__
from mason_cli.config import format_docker_update_message, recommended_update_command_for_method
from mason_cli.web_deps import LateState, late
from mason_cli.web_server_gateway import _ACTION_LOG_FILES
from mason_cli.web_routers._common import http_failure

_log = logging.getLogger("mason_cli.web_server")
router = APIRouter()
status_router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "mason_cli.web_server_files")
_spawn_gateway_restart = late("_spawn_gateway_restart")
_spawn_mason_action = late("_spawn_mason_action", "mason_cli.web_server_gateway")
detect_install_method = late("detect_install_method", "mason_cli.config")
get_mason_home = late("get_mason_home", "mason_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "mason_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "mason_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "mason_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "mason_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import mason_cli.web_server as ws
    import mason_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== mason-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Mason updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== mason update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``mason gateway restart`` in the background."""
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("mason-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "mason-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/mason/update")
async def update_mason():
    """Kick off ``mason update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("mason-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "mason-update", "already_running": True}
        action_id = _ACTION_IDS.get("mason-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn mason update", 500, "Failed to start update"):
        proc = _spawn_mason_action(["update"], "mason-update", env_overrides={"MASON_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "mason-update", "action_id": action_id}
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "Mason is managed by Termux APT; run `pkg upgrade mason-agent`.",
}


@router.get("/api/mason/update/check")
async def check_mason_update(force: bool = False):
    """Report whether a Mason update is available, without applying it.
    ...
    try:
        from mason_cli.banner import check_for_updates

        if force:
            with contextlib.suppress(OSError):
                (get_mason_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "mason-update":
        ...
        if durable_update_action_id:
            marker = f"=== mason-update completed {durable_update_action_id} ==="
...
def _read_latest_receipt() -> Optional[Dict[str, Any]]:
    ...
    try:
        from mason_cli.update_receipt import read_latest_receipt
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``mason update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``mason update`` run since #91283, including
...
@status_router.get("/api/mason/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `mason update` run recorded).")
