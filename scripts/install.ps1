# ============================================================================
# Mason Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$MasonHome = $(if ($env:MASON_HOME) { $env:MASON_HOME } else { "$env:LOCALAPPDATA\mason" }),
    [string]$InstallDir = $(if ($env:MASON_HOME) { "$env:MASON_HOME\mason-agent" } else { "$env:LOCALAPPDATA\mason\mason-agent" }),
    ...
    # builds apps/desktop into a launchable Mason.exe.
    #
    # Why opt-in:
    #   * Mason-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Mason.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Mason.exe
    ...
    #     `mason desktop` already builds on demand.
...
# MASON_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[mason] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'MasonInstall.LongPath').Type) {
            Add-Type -Namespace 'MasonInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [MasonInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -MasonHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('MasonHome')) {
    $MasonHome = ConvertTo-LongPath $MasonHome
} else {
    $MasonHome = ConvertTo-LongPath $(
        if ($env:MASON_HOME) { $env:MASON_HOME } else { "$env:LOCALAPPDATA\mason" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:MASON_HOME) { "$env:MASON_HOME\mason-agent" } else { "$env:LOCALAPPDATA\mason\mason-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: MasonHome=$MasonHome InstallDir=$InstallDir"
}
...
# Detached Mason fork: no official remote exists. Set $env:MASON_REPO_URL to your Mason repo.
$RepoUrlSsh = if ($env:MASON_REPO_URL) { $env:MASON_REPO_URL } else { "git@github.com:Mochi-Sora/Mason-Agent.git" }
$RepoUrlHttps = if ($env:MASON_REPO_URL) { $env:MASON_REPO_URL } else { "https://github.com/Mochi-Sora/Mason-Agent.git" }
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Mason Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $MasonHome)) {
        New-Item -ItemType Directory -Force -Path $MasonHome | Out-Null
    }
    $envFile = Join-Path $MasonHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `mason update`'s npx
    ...
    $prefixDir = Join-Path $MasonHome "node"
...
function Install-Uv {
    # Mason owns its own uv at $MasonHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (mason_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `mason update` stay in sync.
    $managedUv = Join-Path $MasonHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $MasonHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $MasonHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $MasonHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $MasonHome "bin"
        ...
        # (mason_cli/managed_uv.py looks only at $MasonHome\bin\uv.exe).
...
# Put the Mason-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone mason-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Mason-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Mason-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("mason-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and MASON_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Mason belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # mason_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".mason-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Mason-managed interpreter
    ...
    #
    # Under Mason-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Mason-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Mason setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Mason uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\mason\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Mason needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Mason copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\mason\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``MASON_GIT_BASH_PATH`` (User scope) so Mason can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Mason-managed PortableGit install instead..."
    }

    # Download PortableGit into $MasonHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MasonHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Mason features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Mason needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MASON_GIT_BASH_PATH`` (User env scope) so Mason can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MasonHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MasonHome\git\usr\bin\bash.exe
    $candidates += "$MasonHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MasonHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MASON_GIT_BASH_PATH", $candidate, "User")
            $env:MASON_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set MASON_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Mason may not find Git Bash."
    Write-Info "If needed, set MASON_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Mason-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Mason requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Mason requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Mason-managed Node.js installation instead..."

    # Prefer a Mason-managed Node from a previous run over a too-old system one.
    $managedNode = "$MasonHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$MasonHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$MasonHome\node"
        Write-Success "Node.js $version found (Mason-managed)"
        ...
        Update-ManagedNpm "$MasonHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Mason-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $MasonHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MasonHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\mason-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$MasonHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$MasonHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$MasonHome\node") {
                    try {
                        Rename-Item "$MasonHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Mason-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$MasonHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MasonHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\mason-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `mason
                    # update` path (#4735).
                    ...
                    $stashName = "mason-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. mason-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``mason update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Mason behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running mason processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Mochi-Sora/Mason-Agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Mochi-Sora/Mason-Agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Mochi-Sora/Mason-Agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\mason-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\mason-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `mason update` (see the notes at the shared clone-path
    ...
    # next `mason update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Mason-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Mason-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running mason process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running mason processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Mason_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (mason.exe) plus its child tree.
            & taskkill /F /T /IM mason.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM mason.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m mason_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not mason.exe.
            ...
            # Mason children re-exec through .mason-runtime, so killing only
            ...
            # user (see mason_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Mason " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `mason update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Mason (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `mason.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install mason-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and mason.exe broken with
    ...
    # here, not 30 seconds later when the user runs `mason`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Mason processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Mason processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Mason and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register mason.exe in dist-info/RECORD but fail to
        ...
        # `mason` command while mason-agent.exe / mason-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m mason_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `mason dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\mason_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `mason dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `mason dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: mason_cli/web_server.py"
        }
    }
...
function Install-MasonCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the mason launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring mason.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "mason.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the mason command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # mason_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("mason", "mason-acp")) {
    ...
    $requiredExe = Join-Path $Destination "mason.exe"
    $requiredCmd = Join-Path $Destination "mason.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the mason command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up mason command..."
    
    if ($NoVenv) {
        $masonBin = "$InstallDir"
    } else {
        # $MasonHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `mason update`'s autostash
        ...
        # installer staged under mason-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-MasonCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $masonBin = "$MasonHome\bin"
        Install-MasonCommandLaunchers -Root $InstallDir -Destination $masonBin | Out-Null
    }
    ...
    #   mason-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The mason-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept mason via $masonBin)"
        }
    }
    ...
    if ($currentPath -notlike "*$masonBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$masonBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $masonBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set MASON_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\mason instead
    # of the Unix default ~/.mason
    $currentMasonHome = [Environment]::GetEnvironmentVariable("MASON_HOME", "User")
    if (-not $currentMasonHome -or $currentMasonHome -ne $MasonHome) {
        [Environment]::SetEnvironmentVariable("MASON_HOME", $MasonHome, "User")
        Write-Success "Set MASON_HOME=$MasonHome"
    }
    $env:MASON_HOME = $MasonHome
    
    # Update current session
    $env:Path = "$masonBin;$env:Path"
    
    Write-Success "mason command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.mason-bootstrap-complete which tells the Mason
...
function Copy-ConfigTemplates {
    ...
    # Create the MASON_HOME directory structure ($MasonHome, default %LOCALAPPDATA%\mason)
    New-Item -ItemType Directory -Force -Path "$MasonHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MasonHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$MasonHome\.env"
    ...
    # Create config.yaml
    $configPath = "$MasonHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Mason's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$MasonHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in mason_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $MasonHome"
    
    # Seed bundled skills into $MasonHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $MasonHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $MasonHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MasonHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $MasonHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Mason-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'mason setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\mason-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\mason-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\mason-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `mason tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $MasonHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Mason' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Mason' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'mason tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `mason
...
# paths still lazy-install via install_cua_driver() (mason_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `mason computer-use install` runs. Bounded
        ...
        # Mason' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'mason tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: mason computer-use install"
            }
        } else {
...
# the final `electron` -> `Mason` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Mason.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_mason_desktop command
    # resolves apps/desktop/release/win-unpacked/Mason.exe directly,
    ...
    # enforces the supported Node lines and prepends the Mason-managed Node to
    ...
    # The --dir mode produces an unpacked Mason.exe in
    ...
    # are NOT trying to work around). The Mason icon + product name are
    # stamped onto Mason.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\mason-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Mason.exe",
        "$desktopDir\release\win-arm64-unpacked\Mason.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Mason.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Mason icon + identity are stamped onto Mason.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, mason-agent#38216).
    ...
    #    Mason.exe. We deliberately do NOT point them at `mason desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Mason.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Mason.lnk')
        )
        ...
        # with the Mason icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.mason/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'mason setup'."
        return
    }
    ...
    # Run mason setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m mason_cli.main setup
    } else {
        python -m mason_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $masonCmd = "$InstallDir\venv\Scripts\mason.exe"
    if (-not (Test-Path $masonCmd)) {
        $masonCmd = "mason"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$MasonHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'mason whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: mason gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MasonHome\logs\gateway.log"
            Start-Process -FilePath $masonCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MasonHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: mason gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: mason gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MasonHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MasonHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MasonHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MasonHome\mason-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   mason              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   mason setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   mason config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   mason config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   mason gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   mason update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Mason repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.mason-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Mason-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Mason to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
