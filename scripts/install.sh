...
# Python-driven tool session (e.g. Mason terminal tool). A pre-set PYTHONPATH
...
# Configuration — detached Mason fork: no official remote exists.
# Set MASON_REPO_URL to the git URL of YOUR Mason repo.
REPO_URL_SSH="${MASON_REPO_URL:-git@github.com:Mochi-Sora/Mason-Agent.git}"
REPO_URL_HTTPS="${MASON_REPO_URL:-https://github.com/Mochi-Sora/Mason-Agent.git}"
MASON_HOME="${MASON_HOME:-$HOME/.mason}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${MASON_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$MASON_INSTALL_DIR"
...
#   code at /usr/local/lib/mason-agent, command at /usr/local/bin/mason,
#   data still at /root/.mason (MASON_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Mason.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.mason/mason-agent"
            echo "                   default (root, Linux): /usr/local/lib/mason-agent"
            echo "  --mason-home PATH  Data directory (default: ~/.mason, or \$MASON_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Mason installs the code under"
            echo "  /usr/local/lib/mason-agent and links the command into"
            echo "  /usr/local/bin/mason (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$MASON_HOME"
            echo "  (default /root/.mason).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$MASON_HOME/mason-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `mason update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Mason-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Mason Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install mason command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `mason` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $MASON_HOME/mason-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $MASON_HOME/mason-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/mason-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $MASON_HOME/mason-agent — then preserve it)
#
# Always no-op when the user set --dir or $MASON_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in MASON_HOME.
    if is_termux; then
        INSTALL_DIR="$MASON_HOME/mason-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$MASON_HOME/mason-agent/.git" ]; then
            INSTALL_DIR="$MASON_HOME/mason-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/mason-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/mason-agent"
        ...
        # /usr/local/bin/mason wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/mason"
        log_info "  Data:    $MASON_HOME (unchanged)"
...
# Point a Mason-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $MASON_HOME/node/bin — which is NOT on
...
# Mason's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Mason-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$MASON_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$MASON_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$MASON_HOME/node/etc/npmrc"
}

get_mason_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/mason" ]; then
        echo "$link_dir/mason"
    else
        echo "mason"
    fi
...
install_uv() {
    ...
    # Mason owns its own uv at $MASON_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (mason_cli/managed_uv.py) looks in the same
    # place, so install.sh and `mason update` stay in sync.
    local _managed_uv="$MASON_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $MASON_HOME/bin ..."
    mkdir -p "$MASON_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/mason-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/mason-uv-installer.$$.sh")"
    ...
    # directly into $MASON_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$MASON_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Mason currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Mason requires Python >=3.11,<3.14"
...
# replaced with the Mason-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Mason-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Mason-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Mason-managed Node from a previous run over a too-old system one.
    if [ -x "$MASON_HOME/node/bin/node" ] && [ -x "$MASON_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$MASON_HOME/node/bin/node" --version)"; then
        export PATH="$MASON_HOME/node/bin:$PATH"
        log_success "Node.js $("$MASON_HOME/node/bin/node" --version) found (Mason-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Mason-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Mason requires Node 22.22+, 24.11+, or 26+) — installing Mason-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.mason/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.mason/node/..."
    ...
    # Place into ~/.mason/node/ and symlink binaries into the same bin dir
    # the mason command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$MASON_HOME/node"
    mkdir -p "$MASON_HOME"
    mv "$extracted_dir" "$MASON_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$MASON_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$MASON_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$MASON_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$MASON_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.mason/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Mason install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Mason Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `mason update` path
                # (#4735).
                ...
                local stash_name
                stash_name="mason-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``mason update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Mason behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up mason command..."

    if [ "$USE_VENV" = true ]; then
        MASON_BIN="$INSTALL_DIR/venv/bin/python"
        MASON_ENTRYPOINT="$INSTALL_DIR/mason"
    else
        MASON_BIN="$(which mason 2>/dev/null || echo "")"
        if [ -z "$MASON_BIN" ]; then
            log_warn "mason not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$MASON_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$MASON_ENTRYPOINT" ]; }; then
        log_warn "Mason launcher prerequisites not found"
    ...
    # Create a user-facing shim for the mason command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $MASON_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$MASON_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/mason"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/mason" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MASON_BIN" "$MASON_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/mason" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/mason"
    log_success "Installed mason launcher → $command_link_display_dir/mason"

    # Also expose `mason-agent`. The `mason-agent` console script declared in
    ...
    rm -f "$command_link_dir/mason-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/mason-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MASON_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/mason-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MASON_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/mason-agent"
    log_success "Installed mason-agent launcher → $command_link_display_dir/mason-agent"

    # Also expose `mason-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `mason-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Mason as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/mason-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/mason-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MASON_BIN" "$MASON_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/mason-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/mason-acp"
    log_success "Installed mason-acp launcher → $command_link_display_dir/mason-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "mason command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v mason' \
                >/dev/null 2>&1; then
        ...
        log_info "mason not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Mason Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Mason Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Mason Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so mason works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.mason directory structure (config at top level, code in subdir)
    mkdir -p "$MASON_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.mason/.env (top level, easy to find)
    if [ ! -f "$MASON_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$MASON_HOME/.env"
            log_success "Created ~/.mason/.env from template"
        else
            touch "$MASON_HOME/.env"
            log_success "Created ~/.mason/.env"
        fi
    else
        log_info "~/.mason/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$MASON_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.mason/config.yaml (top level, easy to find)
    if [ ! -f "$MASON_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$MASON_HOME/config.yaml"
            log_success "Created ~/.mason/config.yaml from template"
        fi
    else
        log_info "~/.mason/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in mason_cli/default_soul.py — the
    ...
    if [ ! -f "$MASON_HOME/SOUL.md" ]; then
        cat > "$MASON_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.mason/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.mason/"

    # Seed bundled skills into ~/.mason/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `mason update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'mason update'." \
            > "$MASON_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $MASON_HOME/.no-bundled-skills"
        log_info "  Future 'mason update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.mason/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.mason/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$MASON_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$MASON_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.mason/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Mason Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Mason will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Mason Agent browser tools — explicit browser override."
...
# install — the same closure `mason update` installs
# (mason_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `mason update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `mason tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$MASON_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Mason' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$MASON_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'mason tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `mason tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `mason computer-use install` on their own).
    ...
    # Same upstream installer `mason computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Mason install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Mason'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'mason tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: mason computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'mason setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m mason_cli.main setup < /dev/tty
    else
        python -m mason_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$MASON_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Mason to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$MASON_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'mason whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                MASON_CMD="$(get_mason_command_path)"
                $MASON_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'mason whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'mason gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $MASON_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $MASON_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: mason gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: mason gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $MASON_CMD gateway > "$MASON_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.mason/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: mason gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: mason gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.mason-bootstrap-complete, which tells the Mason
    ...
    local marker_path="$INSTALL_DIR/.mason-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $MASON_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $MASON_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $MASON_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}mason${NC}              Start chatting"
    echo -e "   ${GREEN}mason setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}mason config${NC}       View/edit configuration"
    echo -e "   ${GREEN}mason config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}mason gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}mason update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'mason' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'mason' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'mason' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$MASON_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$MASON_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$MASON_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$MASON_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Mason` rename
...
# _purge_electron_build_cache() used by `mason desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Mason-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `mason desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Mason" ]; then
            app="$desktop_dir/release/linux-unpacked/Mason"
        elif [ -x "$desktop_dir/release/linux-unpacked/mason" ]; then
            app="$desktop_dir/release/linux-unpacked/mason"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Mason.app" \
            "$desktop_dir/release/mac/Mason.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `mason desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading MASON_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if MASON_HOME="$MASON_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from mason_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `mason update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Mason-managed Node
            # provisioned during prerequisites/node-deps (at $MASON_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $MASON_HOME.
    # $MASON_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'mason update' on this host install.
