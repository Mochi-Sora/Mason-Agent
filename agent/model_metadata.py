...
from mason_constants import OPENROUTER_MODELS_URL
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    if base_url:
        try:
            from mason_cli.config import get_custom_provider_tls_settings
    ...
    for env_var in ("MASON_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _cache_file(name: str) -> Path:
    ...
    return get_mason_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``mason -q``, cron) start cold and Nous bypasses the persistent context cache, so
...
def _get_context_cache_path() -> Path:
    ...
    from mason_constants import get_mason_home
    return get_mason_home() / "context_length_cache.yaml"
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Mason picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_moa_context_length(model: str, custom_providers: list | None) -> Optional[int]:
    ...
    try:
        from mason_cli.config import get_compatible_custom_providers, load_config
...
def _config_override_context_length(model: str, base_url: str, provider: str, custom_providers: list | None) -> Optional[int]:
    ...
    if custom_providers and base_url and model:
        with contextlib.suppress(Exception):  # fall through to probing
            from mason_cli.config import get_custom_provider_context_length
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from mason_cli.models import get_copilot_model_context
    ...
    if not effective_provider:
        ...
        if model in metadata:
            ...
            if or_ctx == 32768 and _model_name_suggests_stale_32k_underreport(model):
                ...
                return or_ctx
    # 7. Local server before hardcoded defaults — ``Mason-3-Llama-3.1-70B`` matches ``llama``
