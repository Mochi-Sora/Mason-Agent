...
# Defaults for the in-agent progress-aware wrap; mirror mason_cli.config.DEFAULT_CONFIG["compression"] keys.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    Only the exact old ``mason_state.SessionDB`` class (hot-reload skew) may fail open; proxies, lookalikes,
...
def _rebind_session_context(session_id: str) -> None:
    ...
    try:
        ...
        os.environ["MASON_SESSION_ID"] = session_id
    with contextlib.suppress(Exception):
        from mason_logging import set_session_context
...
def _lower_threshold_to_aux_context(
    ...
    if threshold_suggestion_viable:
        ...
        msg += (
            ...
            f"Mason's small-context floor and output reservation would recompute the trigger to "
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "Run `mason setup` or set OPENROUTER_API_KEY."
                )
            agent._compression_warning = msg
            agent._emit_status(msg)
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Mason "
        ...
        if _lock_lookup_error is not None:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "in-memory module after an update; restart the process (or `mason update`) to resync.",
                    _lock_sid,
                )
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from mason_state_common import is_automatic_end_reason
...
def _publish_rotated_compaction(
    ...
    # publish also COALESCEs from the parent row for threads lacking MASON_HOME.
    ...
    agent._session_db.publish_compression_child(
        ...
        source=agent.platform or os.environ.get("MASON_SESSION_SOURCE", "cli"), model=agent.model,
...
def compress_context(
    ...
    # compression.codex_app_server_auto). Memory handoff is Mason-only: no native
...
def _compress_context_via_codex_app_server(
    ...
    Mason' transcript is left unchanged."""
    ...
    if auto_mode not in {"native", "mason", "off"}:
        auto_mode = "native"
    skip_reason = None
    if not force and auto_mode != "mason":
...
def _shrink_data_url(url: str, *, max_dimension: int, resize_fn: Any) -> tuple:
    ...
    try:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="mason_shrink_", suffix=_IMAGE_SUFFIX_BY_MIME.get(mime, ".jpg"), delete=False
        )
        try:
            tmp.write(raw)
            tmp.close()
